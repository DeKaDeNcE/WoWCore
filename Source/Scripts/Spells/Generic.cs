...
class spell_gen_skinning : SpellScript
{
    public override bool Validate(SpellInfo spell)
    {
        return ValidateSpellInfo(SpellIds.ClassicSkinning, SpellIds.OutlandSkinning, SpellIds.NorthrendSkinning, SpellIds.CataclysmSkinning, SpellIds.PandariaSkinning, SpellIds.DraenorSkinning, SpellIds.LegionSkinning, SpellIds.KulTiranSkinning, SpellIds.ZandalariSkinning, SpellIds.ShadowlandsSkinning, SpellIds.DragonIslesSkinning);
    }
    ...
    void HandleSkinningEffect(uint effIndex)
    {
        ...
        ContentTuningRecord contentTuning = CliDB.ContentTuningStorage.LookupByKey(target.GetContentTuning());
        ...
        uint getSkinningLearningSpellBySkill(uint skill)
        {
            switch (skill)
            {
                case (uint)SkillType.OutlandSkinning:
                    return SpellIds.OutlandSkinning;
                case (uint)SkillType.NorthrendSkinning:
                    return SpellIds.NorthrendSkinning;
                case (uint)SkillType.CataclysmSkinning:
                    return SpellIds.CataclysmSkinning;
                case (uint)SkillType.PandariaSkinning:
                    return SpellIds.PandariaSkinning;
                case (uint)SkillType.DraenorSkinning:
                    return SpellIds.DraenorSkinning;
                case (uint)SkillType.KulTiranSkinning:
                    return player.IsTeamAlliance() ? SpellIds.KulTiranSkinning : SpellIds.ZandalariSkinning;
                case (uint)SkillType.ShadowlandsSkinning:
                    return SpellIds.ShadowlandsSkinning;
                case (uint)SkillType.DragonIslesSkinning:
                    return SpellIds.DragonIslesSkinning;
                // Trainer only
                case (uint)SkillType.ClassicSkinning:
                // Quest only
                case (uint)SkillType.LegionSkinning:
                default:
                    break;
            }
