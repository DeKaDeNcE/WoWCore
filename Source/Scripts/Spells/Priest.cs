...
namespace Scripts.Spells.Priest;
    ...
    class spell_pri_aq_3p_bonus : AuraScript
    {
        ...
        void HandleProc(AuraEffect aurEff, ProcEventInfo eventInfo)
        {
            ...
            args.AddSpellBP0((int)MathFunctions.CalculatePct(healInfo.GetHeal(), 10));
    ...
    [Script] // 110744 - Divine Star (Holy)
            ...
            if (spellInfo != null)
            {
                if (spellInfo.GetEffects().Count > 1)
                {
                    ...
                    if (caster != null)
                    {
                        ...
                        firstPath.CalculatePath(destPos.GetPositionX(), destPos.GetPositionY(), destPos.GetPositionZ());
                        ...
                        at.InitSplines(firstPath.GetPath().ToList(), (uint)(at.GetDistance(endPoint.X, endPoint.Y, endPoint.Z) * (1000 / _maxTravelDistance)));
                    }
                }
            }
        }
            ...
            if (caster != null)
            {
                if (!_affectedUnits.Contains(unit.GetGUID()))
                {
                    if (caster.IsValidAttackTarget(unit))
                        caster.CastSpell(unit, at.GetSpellId() == SpellIds.DivineStarShadow ? SpellIds.DivineStarShadowDamage : SpellIds.DivineStarHolyDamage, new CastSpellExtraArgs(TriggerCastFlags.IgnoreGCD | TriggerCastFlags.IgnoreCastInProgress));
                    else if (caster.IsValidAssistTarget(unit))
                        caster.CastSpell(unit, at.GetSpellId() == SpellIds.DivineStarShadow ? SpellIds.DivineStarShadowHeal : SpellIds.DivineStarHolyHeal, new CastSpellExtraArgs(TriggerCastFlags.IgnoreGCD | TriggerCastFlags.IgnoreCastInProgress));
        ...
        void RestoreMana(uint effIndex)
        {
            GetCaster().CastSpell(GetCaster(), SpellIds.PowerWordSolaceEnergize,
                new CastSpellExtraArgs(TriggerCastFlags.IgnoreCastInProgress).SetTriggeringSpell(GetSpell())
                    .AddSpellBP0(GetEffectValue() / 100));
        }
    ...
    class spell_pri_prayer_of_mending : SpellScript
    {
        ...
        void HandleEffectDummy(uint effIndex)
        {
            ...
            args.AddSpellBP0((int)basePoints);
        ...
        void HandleHeal(AuraEffect aurEff, ProcEventInfo eventInfo)
        {
            ...
            if (caster != null)
            {
                ...
                if (stackAmount > 1)
                {
                    ...
                    args.AddSpellBP0(stackAmount - 1);
        ...
        void HandleEffectHit()
        {
            ...
            if (target != null)
            {
                ...
                if (caster.HasAura(SpellIds.MasochismTalent) && caster.GetGUID() == target.GetGUID())
                    caster.CastSpell(caster, SpellIds.MasochismPeriodicHeal, new CastSpellExtraArgs(GetSpell()).AddSpellBP0(periodicAmount));
