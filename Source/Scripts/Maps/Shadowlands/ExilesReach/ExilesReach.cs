...
// MAP 2175 TRANSPORT ALLIANCE
// MAP 2261 TRANSPORT HORDE MAP 2369
// ZONE 10424 https://www.wowhead.com/zone=10424/exiles-reach
// AREA 13377 https://www.wowhead.com/zone=13377/the-north-sea
...
{
    // SparringPartner
    public const uint JumpBehind                            = 312757;
    ...
    // SpellCrashLanded
    public const uint AllianceCrashLanded                   = 305464;
    public const uint HordeCrashLanded                      = 325136;
}
...
{
    // SparringPartner
    public const uint SparringComplete                      = 0;
}
...
{
    // SparringPartner
    public const uint PreFight                              = 14422;
    public const uint Aggro                                 = 14423;
    public const uint Jump                                  = 14424;

    // WarmingUp
    public const uint WarmingUp                             = 12798;
}
...
class npc_sparring_partner : ScriptedAI
{
    ...
    public override void DamageTaken(Unit attacker, ref uint damage, DamageEffectType damageType, SpellInfo spellInfo = null)
    {
        if (me.GetHealth() <= damage)
        {
            ...
            {
                me.SetFacingToObject(player);
                Talk(TextIds.SparringComplete, player);
                player.CastSpell(player, SpellIds.CombatTraining);
            }
        }
        ...
        {
            _jumped = true;
            DoCastVictim(SpellIds.JumpBehind, new CastSpellExtraArgs(true));

            var player = Global.ObjAccessor.GetPlayer(me, _playerGUID);

            if (player != null)
            {
                var convo = Conversation.CreateConversation(ConversationIds.Jump, player, player.GetPosition(), player.GetGUID());

                if (convo != null)
                    convo.AddActor((int)me.GetEntry(), _actorID, me.GetGUID());
            }
        }
    }
    ...
    public override void JustEngagedWith(Unit who)
    {
        ...
        if (player != null && who == player)
        {
            var convo = Conversation.CreateConversation(ConversationIds.Aggro, player, player.GetPosition(), player.GetGUID());
                ...
                case EventIds.MoveToAPosition:
                    {
                        ...
                        _events.ScheduleEvent(EventIds.PreFightConversation, TimeSpan.FromSeconds(1));
                    }
                    break;
                case EventIds.PreFightConversation:
                    var player = Global.ObjAccessor.GetPlayer(me, _playerGUID);

                    if (player != null)
                    {
                        var convo = Conversation.CreateConversation(ConversationIds.PreFight, player, player.GetPosition(), player.GetGUID());
...
[Script] // 305464 Alliance Crash Landed
...
[Script] // 325136 Horde Crash Landed
    ...
    public override void OnQuestStatusChange(Player player, Quest quest, QuestStatus oldStatus, QuestStatus newStatus)
    {
        switch (newStatus)
        {
            case QuestStatus.Complete:
                Conversation.CreateConversation(ConversationIds.WarmingUp, player, player.GetPosition(), player.GetGUID());
                break;
...
{
    quest_brace_for_impact() : base("quest_brace_for_impact") { }
