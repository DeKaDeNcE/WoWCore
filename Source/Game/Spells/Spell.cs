    ...
    public partial class Spell : IDisposable
    {
        ...
        public void UpdateDelayMomentForUnitTarget(Unit unit, ulong hitDelay)
        {
            ...
            else if (m_delayMoment != 0 && oldDelay < hitDelay)
            {
                ...
                var minDelayTargetItr = m_UniqueTargetInfo.MinBy(smallest => itr.TimeDelay < smallest.TimeDelay);

                if (minDelayTargetItr == null)
                    return;
                        ...
                        if (!unitCaster.GetPetGUID().IsEmpty())                  //let warlock do a replacement summon
                        {
                            if (unitCaster.IsTypeId(TypeId.Player))
                            {
                                if (strict)                         //starting cast, trigger pet stun (cast by pet so it doesn't attack player)
                                {
                                    ...
                                    if (pet != null)
                                    {
                                        pet.CastSpell(pet, SharedConst.PetSummoningDisorientation, new CastSpellExtraArgs(TriggerCastFlags.FullMask).SetOriginalCaster(pet.GetGUID()).SetTriggeringSpell(this));
                                    }
                                }
                            }
