        ...
        public void SetSkill(uint id, uint step, uint newVal, uint maxVal)
        {
            ...
            if (skillStatusData != null)
            {
                ...
                else if (currVal != 0 && newVal == 0) // Deactivate skill line
                {
                    ...
                    skillStatusData.State = skillStatusData.State != SkillState.New ? SkillState.Deleted : SkillState.Unchanged; // skills marked as SKILL_NEW don't exist in database (this distinction is not necessary for deletion but for re-learning the same skill before save to db happens)
