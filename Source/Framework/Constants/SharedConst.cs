...
namespace Framework.Constants;
    ...
    public class SharedConst
    {
        ...
        public const float  terminal_length                 = terminalVelocity * terminalVelocity / (2.0f * gravity);
        public const float  terminal_safeFall_length        = terminalSafefallVelocity * terminalSafefallVelocity / (2.0f * gravity);
        public const float  terminal_fallTime               = terminalVelocity / gravity; // the time that needed to reach terminalVelocity
        public const float  terminal_safeFall_fallTime      = terminalSafefallVelocity / gravity; // the time that needed to reach terminalVelocity with safefall
        ...
        public static SkillType SkillByQuestSort(int sort)
        {
            return (QuestSort)sort switch
            {
                ...
                QuestSort.Jewelcrafting  => SkillType.Jewelcrafting,
                QuestSort.Inscription    => SkillType.Inscription,
                QuestSort.Archaeology    => SkillType.Archaeology,
                _ => SkillType.None
            };
        }
        ...
        public static bool IsValidLocale(Locale locale) => locale < Locale.Total && locale != Locale.None;
        ...
        static int GetRaceBit(Race raceId) => raceId switch
        {
            Race.Human              => (int)raceId - 1,
            Race.Orc                => (int)raceId - 1,
            Race.Dwarf              => (int)raceId - 1,
            Race.NightElf           => (int)raceId - 1,
            Race.Undead             => (int)raceId - 1,
            Race.Tauren             => (int)raceId - 1,
            Race.Gnome              => (int)raceId - 1,
            Race.Troll              => (int)raceId - 1,
            Race.Goblin             => (int)raceId - 1,
            Race.BloodElf           => (int)raceId - 1,
            Race.Draenei            => (int)raceId - 1,
            Race.Worgen             => (int)raceId - 1,
            Race.PandarenNeutral    => (int)raceId - 1,
            Race.PandarenAlliance   => (int)raceId - 1,
            Race.PandarenHorde      => (int)raceId - 1,
            Race.Nightborne         => (int)raceId - 1,
            Race.HighmountainTauren => (int)raceId - 1,
            Race.VoidElf            => (int)raceId - 1,
            Race.LightforgedDraenei => (int)raceId - 1,
            Race.ZandalariTroll     => (int)raceId - 1,
            Race.KulTiran           => (int)raceId - 1,
            Race.DarkIronDwarf      => 11,
            Race.Vulpera            => 12,
            Race.MagharOrc          => 13,
            Race.MechaGnome         => 14,
            Race.DracthyrAlliance   => 16,
            Race.DracthyrHorde      => 15,
            _ => -1
        };
        ...
        public static bool IsActivePetSlot(PetSaveMode slot) => slot >= PetSaveMode.FirstActiveSlot && slot < PetSaveMode.LastActiveSlot;

        public static bool IsStabledPetSlot(PetSaveMode slot) => slot >= PetSaveMode.FirstStableSlot && slot < PetSaveMode.LastStableSlot;

        public static LootType GetLootTypeForClient(LootType lootType) => lootType switch
        {
            LootType.Prospecting => LootType.Disenchanting,
            LootType.Milling     => LootType.Disenchanting,
            LootType.Insignia    => LootType.Skinning,
            LootType.FishingHole => LootType.Fishing,
            LootType.FishingJunk => LootType.Fishing,
            _ => lootType
        };
    }
    ...
    public enum Class
    {
        ...
        ClassMaskAllPlayable  = (1 << (Warrior - 1)) | (1 << (Paladin - 1)) | (1 << (Hunter - 1)) | (1 << (Rogue - 1)) | (1 << (Priest - 1)) | (1 << (Deathknight - 1)) | (1 << (Shaman - 1)) | (1 << (Mage - 1)) | (1 << (Warlock - 1)) | (1 << (Monk - 1)) | (1 << (Druid - 1)) | (1 << (DemonHunter - 1)) | (1 << (Evoker - 1)),
    ...
    {
        TalkUsePlayer           = 0x1, // Talk Flags
        EmoteUseState           = 0x1, // Emote Flags
        TeleportUseCreature     = 0x1, // Teleport Flags
        KillCreditRewardGroup   = 0x1, // KillCredit Flags
        RemoveAuraReverse       = 0x1, // RemoveAura Flags
        CastSpellSourceToTarget = 0,   // CastSpell Flags
        ...
        PlaySoundTargetPlayer   = 0x1, // PlaySound Flags
        PlaySoundDistanceSound  = 0x2,
        OrientationFaceTarget   = 0x1  // Orientation Flags
    }
    ...
    public enum ScriptCommands
    {
        ...
        Equip                = 31, // Source = Creature, Datalong = Equipment Id
    ...
    public enum WorldCfg
    {
        ...
        GameObjectCheckInvalidPosition,
        GameType,
        GmChat,
        GmFreezeDuration,
        GmLevelInGmList,
    ...
    public enum GameError : uint
    {
        ...
        EmblemErrorNotAboardGeoset                           = 379,
        SpellOutOfRange                                      = 380,
        ...
        MailRecipientCantReceiveMail                         = 446,
        MailSent                                             = 447,
        ...
        BattlegroundQueuedForRated                           = 576,
        ...
        ItemUniqueEquipable                                  = 609,
        LfgLeaderIsLfmS                                      = 610,
        LfgPending                                           = 611,
        CantSpeakLanguage                                    = 612,
        ...
        ItemUniqueEquipableSocketed                          = 629,
        UserSquelched                                        = 630,
        AccountSilenced                                      = 631,
